...
export async function SendSignInEmail(
  ...
  try {
    ...
    return res.status(404).send({
      status: "Error",
      message:
        "We had problems sending the email with your api_key. Please verify your email address"
    });
  }
}

export async function VerifyUserAlreadyExists(
  req: Request,
  res: Response,
  next: NextFunction
): Promise<Response | void> {
  const user: IDBResponse<IUserEntity[]> = await UserRepository.GetByEmail(
    req.body.email
  );
...
export async function HandleUpdateUserValidation(
  req: Request,
  res: Response,
  next: NextFunction
) {
  ...
  if (dbUser.status === Status.Successfull) {
    const passwordIsValid: boolean = await bcrypt.compare(
      req.body.password,
      dbUser.message[0].password
    );
    const emailIsValid: boolean = UserValidators.SafeCompare(
      dbUser.message[0].email,
      req.body.email
    );
