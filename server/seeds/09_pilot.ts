...
const buildPilotRelation = async (vehicles: IVehicleFromApi[]) =>
  Promise.all(
    ...
    .then((vehicle: { vehicle_id: Array<{ id: string }>; people: any[] }[]) =>
      vehicle.map((obj) => ({
        vehicle_id: obj.vehicle_id[0].id,
        people: flatten(obj.people).map((v) => v && v.id)
      }))
    )
    .then((obj: { vehicle_id: string; people: string[] }[]) =>
      flatten(
        obj.map(({ vehicle_id, people }) =>
          people.reduce((acc: any, people_id: string) => [...acc, { id: uuid(), vehicle_id, people_id }], [])
        )
      )
    )
    .catch((e) => e);
