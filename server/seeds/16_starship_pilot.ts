...
import { StarshipPilot } from '../models/StarshipPilot';
...
export async function seed(knex: Knex): Promise<any> {
  ...
  return knex(ManyToManyTable.StarshipPilot)
    .del()
    .then(() => {
...
const buildStarshipPilot = async (starshipFromApi: IStarshipFromApi[]): Promise<StarshipPilot[]> =>
  await Promise.all(
    starshipFromApi.map(async (starship: IStarshipFromApi) => ({
      starship_id: await knex
